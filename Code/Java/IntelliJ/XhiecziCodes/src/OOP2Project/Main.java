...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        // Seed a sample route (demo) — Operator can add more later
    ...
    private static void registerUser(Scanner sc, AuthenticationService auth) {
        ...
        switch (roleChoice) {
            case "1":
                ...
                System.out.println("Operator account created successfully!");
                break;
            case "2":
                newUser = new Commuter(userId, name, username, password);
                System.out.println("Commuter account created successfully!");
                break;
            default:
    ...
    private static void operatorMenu(Scanner sc, MonitoringService monitoring) {
        while (true) {
            ...
            System.out.println("5) Send Vehicle Ping (simulate GPS)");
    ...
    private static void commuterMenu(Scanner sc, MonitoringService monitoring) {
        while (true) {
            System.out.println("\n--- COMMUTER MENU ---");
            System.out.println("1) View Route Status (ETAs + vehicles)");
    ...
    private static void addRoute(Scanner sc, MonitoringService monitoring) {
        ...
        monitoring.addRoute(route);
        System.out.println("Added route: " + id + " - " + name);
    }
    ...
    private static void addStop(Scanner sc, MonitoringService monitoring) {
        ...
        System.out.println("Added stop to route " + route.routeId() + ": " + sname);
    }

    private static void registerVehicle(Scanner sc, MonitoringService monitoring) {
        System.out.println("Vehicle type: 1) Jeepney  2) ModernJeep  3) Bus");
        ...
        System.out.println("Registered " + v.getVehicleType() + " with ID " + vid);
    }
    ...
    private static void assignVehicleToRoute(Scanner sc, MonitoringService monitoring) {
        ...
        System.out.println("Assigned vehicle " + v.vehicleId() + " to route " + r.routeId());
    }
    ...
    private static void sendPing(Scanner sc, MonitoringService monitoring) {
        ...
        System.out.println("Ping received for " + v.vehicleId());
    }
    ...
    private static void viewDashboard(Scanner sc, MonitoringService monitoring) {
        ...
        System.out.println("\n=== ROUTE DASHBOARD: " + r.routeId() + " - " + r.routeName() + " ===");
        List<PublicVehicle> vehicles = monitoring.getVehiclesByRoute(r.routeId());

        if (vehicles.isEmpty()) {
            System.out.println("No vehicles assigned / active on this route.");
            return;
        }

        for (PublicVehicle v : vehicles) {
            if (!v.hasLocation()) {
                System.out.println(v.vehicleId() + " (" + v.getVehicleType() + ") - NO DATA YET");
                continue;
            }
            ...
            System.out.printf(
                    "%s (%s) plate=%s pax=%d/%d speed=%.1fkm/h%n",
                    v.vehicleId(), v.getVehicleType(), v.plateNumber(),
                    v.passengerCount(), v.capacity(), v.speedKmh()
            );
            System.out.printf(
                    "  Location: (%.5f, %.5f) | Nearest Stop: %s | ETA: %.1f min%n",
                    v.lat(), v.lon(), (nearest != null ? nearest.stopName() : "N/A"), etaMin
            );
        }
    }

    private static void viewAlerts(MonitoringService monitoring) {
        System.out.println("\n=== ALERTS LOG (latest first) ===");
        ...
        for (Alert a : alerts) {
            ...
            shown++;
            if (shown >= 20) break; // show latest 20
        }
    }
    ...
    private static PublicVehicle pickVehicle(Scanner sc, MonitoringService monitoring) {
        ...
        for (int i = 0; i < vehicles.size(); i++) {
            ...
            System.out.println((i + 1) + ") " + v.vehicleId() + " (" + v.getVehicleType() + ") plate=" + v.plateNumber());
        }
